  ...
  methods: {
    format(command) {
      ...
      if (command.usage !== usage)
        output.push(
          `> ${this.$t(
            "website.commands.components.usage"
          )} \`${command.name} ${usage}\``
        );
      if (command.aliases.length > 0)
        output.push(
          `> ${this.$t(
            "website.commands.components.aliases"
      ...
      if (command.flags.length > 0) {
        ...
        for (const flag of command.flags)
          output.push(
            ` - \`-${flag.flag} | --${flag.name}\` - ${this.$t(flag.info)}`
          );
      }
      if (command.subcommands.length > 0) {
        output.push(
          `##### ${this.$t("website.commands.components.subcommands")}`
        );
