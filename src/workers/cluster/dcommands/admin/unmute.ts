...
export class UnmuteCommand extends BaseGuildCommand {
    ...
    public async unmute(context: GuildCommandContext, userStr: string, flags: FlagResult): Promise<string> {
        ...
        switch (await context.cluster.moderation.mutes.unmute(member.value, context.author, reason)) {
            case 'notMuted': return this.error(`${humanize.fullName(member.value.user)} is not currently muted`);
            ...
            case 'success': return this.success(`**${humanize.fullName(member.value.user)}** has been muted`);
        }
    }
}
