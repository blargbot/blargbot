...
export class DonateCommand extends BaseGlobalCommand {
    ...
    public async donateDetails(context: CommandContext): Promise<string> {
        ...
        await context.replyDM({
            embeds: [
                {
                    author: owner === null ? undefined
                        : 'members' in owner ? {
                            name: owner.name,
                            iconURL: owner.iconURL({ format: 'png', size: 512 }) ?? undefined,
                            url: context.util.websiteLink('donate')
                        } : {
                            name: owner.username,
                            iconURL: owner.displayAvatarURL({ dynamic: true, size: 512, format: 'png' }),
                            url: context.util.websiteLink('donate')
                        },
