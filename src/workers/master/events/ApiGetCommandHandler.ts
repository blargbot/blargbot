...
export class ApiGetCommandHandler extends WorkerPoolEventService<ApiConnection> {
    private nextCluster: number;

    public constructor(private readonly master: Master) {
        super(master.api, 'getCommand');
        this.nextCluster = 0;
    }
    ...
    protected async execute(...args: Parameters<WorkerPoolEventHandler<ApiConnection>>): Promise<void> {
        ...
        const result = await cluster.request('getCommand', args[1]);
        args[3](result);
    }
}
