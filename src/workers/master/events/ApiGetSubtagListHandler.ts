...
export class ApiGetSubtagListHandler extends WorkerPoolEventService<ApiConnection> {
    private nextCluster: number;

    public constructor(private readonly master: Master) {
        super(master.api, 'getSubtagList');
        this.nextCluster = 0;
    }
    ...
    protected async execute(...args: Parameters<WorkerPoolEventHandler<ApiConnection>>): Promise<void> {
        ...
        const result = await cluster.request('getSubtagList', undefined);
        args[3](result);
    }
}
