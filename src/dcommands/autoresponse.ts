...
export class AutoResponseCommand extends BaseGuildCommand {
    ...
    public async addAutoresponse(context: GuildCommandContext, pattern: string, isRegex: boolean, isEverything: boolean): Promise<string> {
        ...
        let commandName;
        const ars = await this.cluster.database.guilds.getAutoresponses(context.channel.guild.id);
        const commands = await this.cluster.database.guilds.listCommands(context.channel.guild.id);
        ...
        if (isEverything) {
            ...
            await this.cluster.database.guilds.setAutoresponse(context.channel.guild.id, 'everything', { executes: commandName });
        } else {
            ...
            if (isRegex) {
                const regexError = this.validateRegex(pattern);
                if (regexError)
                    return regexError;
            }
            while (commandNames.has(commandName = `_autoresponse_${commandIndex++}`));
            await this.cluster.database.guilds.addAutoresponse(context.channel.guild.id, { executes: commandName, regex: isRegex, term: pattern });
        }
    ...
    public async removeAutoresponse(context: GuildCommandContext): Promise<string | undefined> {
        ...
        const match = await this.cluster.database.guilds.getAutoresponse(context.channel.guild.id, ar.index);
        await this.cluster.database.guilds.setAutoresponse(context.channel.guild.id, ar.index, undefined);
        await this.cluster.database.guilds.setCommand(context.channel.guild.id, ar.executes, undefined);
    ...
    public async editAutoresponse(context: GuildCommandContext, pattern: string, isRegex: boolean, isEverything: boolean): Promise<string | undefined> {
        ...
        await this.cluster.database.guilds.setAutoresponse(context.channel.guild.id, ar.index, {
    ...
    private validateRegex(pattern: string): string | undefined {
        ...
        this.logger.log('Testing the regex', result.regex, 'with the following phrases:\n', testPhrases);
    ...
    private async requestEditableAutoresponse(context: GuildCommandContext, includeEverything: boolean): Promise<string | undefined | { index: number | 'everything', executes: string }> {
        ...
        const result = await this.cluster.util.awaitQuery(
            context.channel,
            context.author,
            `Autoresponses:
