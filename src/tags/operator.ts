...
export class OperatorSubtag extends BaseSubtag {
    ...
    public enrichDocs(
        ...
        const comparisonOperationDesc = `\`${Object.keys(compare).join(', ')}\`` +
            '\nComparison operators behave in a similar way to `{bool}`, but can accept more than two values. If an argument is an array, this array will be flattened (except for `startswith, includes, contains and endswith`). Because it an accept more than two values the logic is a little different. `{==;1;2;3;4}` would mean `1 == 2 && 2 == 3 && 3 == 4`. For `startswith, includes, contains and endswith` this translates to the following:\n- `{includes;abc;a;b;c} = "abc".includes("a") && "abc".includes("b") && "abc".includes("c")`\n**Examples**:```\n' +
