    ...
    return {
        ...
        * resolve(context, call) {
            ...
            for (const item of parameterMap) {
                const arg = call.args[item.argIndex] as BBTagExpression | undefined;
                if (arg === undefined)
                    yield item.default;
                else {
                    args.delete(arg);
...
function createVariableResolver<Locals extends Record<string, unknown>>(
    ...
    const resolverCache = {} as Record<number, ArgumentResolver<Locals> | undefined>;
