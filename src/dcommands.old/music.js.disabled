...
var youtubeStream = dep.youtubeStream;
var google = dep.google;
...
if (dep.fs.existsSync(dep.path.join(__dirname, '../voiceSettings.json'))) {
    var voiceFile = dep.fs.readFileSync(dep.path.join(__dirname, '../voiceSettings.json'), 'utf8');
...
function saveVoiceSettings() {
    dep.fs.writeFile(dep.path.join(__dirname, '../voiceSettings.json'), JSON.stringify(voiceSettings, null, 4));
}
...
e.execute = (msg, words) => {
    ...
    bot.sendChannelTyping(msg.channel.id).then(() => {
        ...
        console.music(`${msg.channel.guild.name} (${msg.channel.guild.id})> ${msg.channel.name} (${msg.channel.id}> ${msg.author.username} (${msg.author.id})> ${msg.content}`);
                                ...
                                bu.guildSettings.get(msg.channel.guild.id, 'musicchannel').then(channel => {
                                    if (channel == msg.channel.id) {
                                        bu.guildSettings.remove(msg.channel.guild.id, 'musicchannel').then(() => {
                                            bu.send(msg, 'This is no longer my music channel.');
                                        });
                                    } else {
                                        bu.guildSettings.set(msg.channel.guild.id, 'musicchannel', msg.channel.id);
                                        bu.send(msg, 'This is now my music channel.');
                                    }
                                });
            ...
            case 'skip':
                ...
                if (voiceConnections.get(msg.channel.guild.id).ready) {
                    ...
                    if (current[msg.channel.guild.id].votes.indexOf(msg.author.id) > -1) {
                        bu.send(msg, `:no_good: You've already voted to skip! :no_good: `);
                        return;
                    }
                    current[msg.channel.guild.id].votes.push(msg.author.id);

                    if (current[msg.channel.guild.id].votes.length >= votesNeeded) {
                        bu.send(msg, `:umbrella2: Skipping the song \`${cache[current[msg.channel.guild.id].id].name}\` after ${votesNeeded} votes. :umbrella2:`);
                        voiceConnections.get(msg.channel.guild.id).stopPlaying();

                    } else {
                        bu.send(msg, `:closed_umbrella: ${msg.member.nick
                            ? msg.member.nick
...
Type ${prefix ? prefix : config.discord.defaultPrefix}music for music commands.`
                                ...
                                voice.on('connect', () => {
                                    try {
                                        console.music(`Connected to guild ${msg.channel.guild.name} (${msg.channel.guild.id}) in channel ${bot.getChannel(bot.voiceConnections[msg.channel.guild.id].channelID).name} (${bot.voiceConnections[msg.channel.guild.id].channelID})`);
                                    } catch (err) {
                                        console.error(err);
                                    }
                                });
                                voice.on('ready', () => {
                                    try {
                                        console.music(`Ready to guild ${msg.channel.guild.name} (${msg.channel.guild.id}) in channel ${bot.getChannel(bot.voiceConnections[msg.channel.guild.id].channelID).name} (${bot.voiceConnections[msg.channel.guild.id].channelID})`);
                                    } catch (err) {
                                        console.error(err);
                                    }
                                });
                                ...
                                voice.on('end', function() {
                                    try {
                                        console.music(`Finished stream in guild ${msg.channel.guild.name} (${msg.channel.guild.id})`);
                                ...
                                voice.on('disconnect', function() {
                                    console.music(`Disconnected from guild ${msg.channel.guild.name} (${msg.channel.guild.id}) in channel ${bot.getChannel(msg.member.voiceState.channelID).name} (${msg.member.voiceState.channelID})`);
                    ...
                    switch (words[0]) {
                        case 'shuffle':
                            for (i = 0; i < queue[msg.channel.guild.id].length; i++) {
                                //     console.(cache[queue[msg.channel.guild.id][i].id].name);
                            }
                            //   console.('------------------------------------------------------');
                            ...
                            //    console.(dep.util.inspect(queue[msg.channel.guild.id]))
                            ...
                            if (bu.hasPerm(msg, 'Bot Commander')) {
                                if (words[1]) {
                                    ...
                                    if (queue[msg.channel.guild.id][index]) {
                                        ...
                                        //        console.(dep.util.inspect(removed))
                                        bu.send(msg, `:umbrella: Removed the song **${removedSong}** :umbrella:`);

                                    }
                                }
                            } else {
                                bu.send(msg, `:no_good: You don't have permissions to remove a song from the queue :no_good:`);
                            }
                            break;
                    }
                    // console.(dep.util.inspect(queue[msg.channel.guild.id]))
...
function sendQueue(msg) {
    ...
    if (current[msg.channel.guild.id]) {
        ...
        var timeDiff = dep.moment.duration(dep.moment().diff(dep.moment(currentSong.start)));
        var timeLength = dep.moment.duration(cache[currentSong.id].duration);
        console.music(currentSong.requester);
    ...
    if (queue.hasOwnProperty(msg.channel.guild.id) && queue[msg.channel.guild.id].length > 0) {
        for (var i = 0; i < (queue[msg.channel.guild.id].length <= 10 ? queue[msg.channel.guild.id].length : 10); i++) {
            ...
            line += name;
            line = pad(line, 51);
            line += ` - [${createTimeString(dep.moment.duration(cache[id].duration))}] (${requester})\n`;
            oddApo = (line.match(/'/g) || []).length % 2;
            messageToSend += oddApo == 0 ? line : line.replace(/'/, '\u2019');
        }
...
function eval2(msg, text) {
    if (msg.author.id === CAT_ID) {
        ...
        console.music(commandToProcess);
        try {
...
function init() {
    ...
    if (dep.fs.existsSync(dep.path.join(__dirname, '..', 'keys.json'))) {
    ...
    if (!dep.fs.existsSync(dep.path.join(__dirname, '..', 'cache', 'yt'))) {
        dep.fs.mkdirSync(dep.path.join(__dirname, '..', 'cache', 'yt'));
    }
    if (!dep.fs.existsSync(dep.path.join(__dirname, '..', 'cache', 'sc'))) {
        dep.fs.mkdirSync(dep.path.join(__dirname, '..', 'cache', 'sc'));
    }
    if (dep.fs.existsSync(dep.path.join(__dirname, '..', 'cache', 'cache.json'))) {
        var cacheFile = dep.fs.readFileSync(dep.path.join(__dirname, '..', 'cache', 'cache.json'), 'utf8');
        cache = JSON.parse(cacheFile);
        cache = cache;
    } else {
        saveCache();
    }
    //  console.(dep.util.inspect(keys))
}

function saveCache() {
    dep.fs.writeFile(dep.path.join(__dirname, '..', 'cache', 'cache.json'), JSON.stringify(cache, null, 4));
}
        ...
        if (/https:\/\/soundcloud.com/.test(query)) { //SC playlist
            ...
            if (/v=(.+?)(&|$)/.test(query)) {
                ...
                youtube.playlists.list({
                    ...
                    console.music(dep.util.inspect(res));
...
function saveSoundcloud(msg, id, callback) {
    ...
    var writeStream = dep.fs.createWriteStream(cache[id].path);
        ...
        switch (type) {
            case 'playlists':
                SC.get(`/playlists/${id}`, (err, track) => {
                    ...
                    //     console.(track)
                    var added = 0;
                    bu.send(msg, 'Processing playlist...').then((msg2) => {

                        for (var i = 0; i < track.tracks.length; i++) {
                            var curTrack = track.tracks[i];
                            console.music(curTrack.id);
                            if (curTrack.streamable) {
                                if (!cache[curTrack.id]) {
                                    cache[curTrack.id] = {
                                        name: curTrack.title,
                                        path: dep.path.join(__dirname, '..', 'cache', 'sc', `${curTrack.id}.mp3`),
                                        duration: dep.moment.duration(curTrack.duration).toJSON(),
                                        sc: true
                                    };
                                }
            ...
            case 'tracks': //msg, id, name, duration, sc
                SC.get(`/tracks/${id}`, (err, track) => {
                    if (err) {
                        console.music('Nothing found');
                        bu.send(msg, `No results found.`);
                        return;
                    }
                    console.music(track);
                    if (!track.streamable) {
                        bu.send(msg, `I can't play that song!`);
                        return;
                    }
                    var duration = dep.moment.duration(track.duration).toJSON();
    ...
    bu.guildSettings.get(msg.channel.guild.id, 'musicchannel').then(channel => {
        ...
        saveVideo(msg, channel, nextSong.id, cache[nextSong.id].name, cache[nextSong.id].duration, () => {
            ...
            try {
                ...
                voiceConnections.get(msg.channel.guild.id).play(cache[nextSong.id].path, {
                    inlineVolume: voiceSettings[msg.channel.guild.id].volume / 100
                });
                voiceConnections.get(msg.channel.guild.id).setVolume(voiceSettings[msg.channel.guild.id].volume / 100);
                currectNext.start = dep.moment();
...
function saveVideo(msg, channel, id, name, duration, callback) {
    ...
    if (!cache[id].sc) {
        ...
        console.music(url);
        var filepath = dep.path.join(__dirname, '..', 'cache', 'yt', `${id}.mp3`);
        if (!dep.fs.existsSync(filepath)) {
            bot.createMessage(channel, `:cd: Downloading song \`${name}\`... :cd: `).then((newmessage) => {
                var stream = getStreamFromURL(url);
                //  console.(dep.util.inspect(stream));
                var writeStream = dep.fs.createWriteStream(filepath);
        ...
        if (!dep.fs.existsSync(cache[id].path))
            bot.createMessage(channel, `:cd: Downloading song \`${name}\`... :cd: `).then((newmessage) => {
...
function addToQueue(msg, id, name, duration, sc) {
    ...
    //   console.(dep.util.inspect(connections))
    ...
    var subqueue = queue[msg.channel.guild.id];
    var init = dep.moment.duration(0);
    //  console.(init.hours(), init.minutes(), init.seconds())

    for (var i = 0; i < subqueue.length; i++) {
        init.add(dep.moment.duration(cache[subqueue[i].id].duration));
        console.music(i, ':', init.hours(), init.minutes(), init.seconds());
    }
    if (current[msg.channel.guild.id]) {
        init.add(dep.moment.duration(cache[current[msg.channel.guild.id].id].duration));
        init.subtract(dep.moment.duration(dep.moment().diff(current[msg.channel.guild.id].start)));
    }
    ...
    if (!cache[id]) {
        cache[id] = {
            name: name,
            path: dep.path.join(__dirname, '..', 'cache', sc ? 'sc' : 'yt', `${id}.mp3`),
            duration: duration,
            sc: sc
            //   requester: msg.author.id
        };
...
function findVideo(msg, text, callback) {
    try {
        youtube.search.list({
            ...
            //     console.(dep.util.inspect(res))
            //      console.(dep.util.inspect(res.items[0].id))
            //    console.(dep.util.inspect(res.items[0].snippet))
...
function addPlaylistToQueue(msg, id, res) {
    ...
    bu.send(msg, `Processing playlist \`${playlist.snippet.title}\` with ${playlist.contentDetails.itemCount} items.`).then((msg2) => {
        console.music('starting processplaylist');

        processPlaylist([], msg.author.id, id, playlist, null, (newQueue) => {
            console.music('processplaylist done');
            saveCache();
            //  console.(dep.util.inspect(newQueue))
...
function processPlaylist(subqueue, requesterid, id, playlist, nextPageToken, callback) {
    var onComplete = function(newsubqueue, newpagetoken) {
        console.music('done a batch', newsubqueue.length, '/', playlist.contentDetails.itemCount);
    ...
    function doThing(err, res2) {
        ...
        cache[res2.items[0].id] = {
            ...
            id: res2.items[0].id,
            path: dep.path.join(__dirname, '..', 'cache', 'yt', `${res2.items[0].id}.mp3`),
