...
export class TimeoutManager extends ModerationManagerBase {
    ...
    private readonly ignoreTimeoutClears: Set<`${string}:${string}`>;
    ...
    private async updateUserTimeoutDate(guild: Guild, userId: string, moderator: User, authorizer: User, duration: Duration, reason?: string): Promise<TimeoutResult | { error: unknown; }> {
        const self = guild.members.get(this.cluster.discord.user.id);
        if (self?.permissions.has('moderateMembers') !== true) {
            return 'noPerms';
        }
