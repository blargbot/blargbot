...
\`\`\``)
    ...
    subtags: {
        ...
        get: {
            ['0']: {
                description: t('Returns the stored variable `varName`.\nYou can use a character prefix to determine the scope of your variable.\nValid scopes are: `_` ([object Object]), `@` ([object Object]), `*` ([object Object]), `~` ([object Object]), no prefix ([object Object]). For more information, use `b!t docs variable` or `b!cc docs variable`'),
        ...
        set: {
            ...
            ['1']: {
                description: t('Stores `value` under `name`. These variables are saved between sessions. You can use a character prefix to determine the scope of your variable.\nValid scopes are: `_` ([object Object]), `@` ([object Object]), `*` ([object Object]), `~` ([object Object]), no prefix ([object Object]).\nFor performance reasons, variables are not immediately stored to the database. See `\\{commit\\}` and `\\{rollback\\}`for more information, or use `b!t docs variable` or `b!cc docs variable`'),
                exampleOut: t('This is local var1\nThis is temporary var2'),
                exampleCode: t('\\{set;var1;This is local var1\\}\n\\{set;~var2;This is temporary var2\\}\n\\{get;var1\\}\n\\{get;~var2\\}')
            },
            ['2']: {
        ...
        channeltype: {
            description: t('Possible results: `text`, `dm`, `voice`, `group-dm`, `category`, `news`, `store`, `news-thread`, `public-thread`, `private-thread`, `stage-voice`'),
            ['0']: {
        ...
        slowmode: {
            ...
            ['1']: {
                description: t('Removes slowmode from `channel`. If `channel` cannot be resolved directly, it will enable slowmode for the current channel and set the cooldown to `time`'),
                exampleOut: t('(disabled slowmode in testing-grounds)\n(set slow mode to 10 seconds)'),
                exampleCode: t('\\{slowmode;testing-grounds\\}\n\\{slowmode;10\\}')
            },
            ['2']: {
                description: t('Enables slowmode in `channel` and set the cooldown to `time`. If `channel` cannot be resolved directly, it will remove slowmode in the current channel (unless `channel` is a valid number, then the slowmode cooldown will be set to `channel` in the current channel).'),
        ...
        emojicreate: {
            ['0']: {
                description: t('Creates a emoji with the given name and image. `image` is either a link to an image, or a base64 encoded data url (`data:<content-type>;base64,<base64-data>`). You may need to use \\{semi\\} for the latter. Returns the new emojis\'s ID.'),
                exampleOut: t('11111111111111111'),
                exampleCode: t('\\{emojicreate;fancy_emote;https://some.cool/image.png\\}')
            },
            ['1']: {
        ...
        while: {
            ...
            ['1']: {
                description: t('This will continuously execute `code` for as long as the condition returns `true`. The condition is as follows:\nIf `evaluator` and `value2` are provided, `value1` is evaluated against `value2` using `evaluator`. Valid evaluators are `==`, `!=`, `>=`, `>`, `<=`, `<`, `startswith`, `endswith`, `includes`, `contains`.'),
        ...
        math: {
            ['0']: {
                description: t('Accepts multiple `values` and returns the result of `operator` on them. Valid operators are `+`, `-`, `*`, `/`, `%`, `^`\nSee `\\{operators\\}` for a shorter way of performing numeric operations.'),
        ...
        edit: {
            ...
            ['0']: {
                description: t('Edits `messageID` in the current channel to say `text` or `embed`'),
                exampleOut: t(''),
            ...
            ['2']: {
                description: t('Edits `messageID` in `channelID` to say `text` or `embed`'),
                exampleOut: t(''),
        ...
        bool: {
            ['0']: {
                description: t('Evaluates `arg1` and `arg2` using the `evaluator` and returns `true` or `false`. Valid evaluators are `==`, `!=`, `>=`, `>`, `<=`, `<`, `startswith`, `endswith`, `includes`, `contains`\nThe positions of `evaluator` and `arg1` can be swapped.'),
                exampleOut: t('true'),
        ...
        hash: {
            ...
            ['1']: {
                description: t('Perfoms a hash on the given `text`. If the `text` starts with `buffer:` then it will first be decoded as a base64 string. If it starts with `text:` then it will be treated as plaintext. The hash result will be returned as a hex number.\nSupported `algorithm`s are: `md5`, `sha1`, `sha256`, `sha512`, `whirlpool`'),
        ...
        if: {
            description: t('If `evaluator` and `value2` are provided, `value1` is evaluated against `value2` using `evaluator`. If they are not provided, `value1` is read as `true` or `false`. If the resulting value is `true` then the tag returns `then`, otherwise it returns `else`.\nValid evaluators are `==`, `!=`, `>=`, `>`, `<=`, `<`, `startswith`, `endswith`, `includes`, `contains`.'),
            ['0']: {
        ...
        logic: {
            ['0']: {
                description: t('Accepts 1 or more boolean `values` (`true` or `false`) and returns the result of `operator` on them. Valid logic operators are `&&`, `||`, `xor`, `!`, `^`.See `\\{operators\\}` for a shorter way of performing logic operations.'),
                exampleOut: t('false'),
        ...
        dm: {
            ['0']: {
                description: t('DMs `user` the given `message|embed`. If `message|embed` is a valid embed, it will be treated as embed. You may only send one DM per execution. Requires author to be staff, and the user to be on the current guild.\nPlease note that `embed` is the JSON for an embed object, don\'t put the `\\{embed\\}` subtag there, as nothing will show.'),
        ...
        useractivitytype: {
            description: t('Activity types can be any of `playing, streaming, listening, watching, custom, competing`'),
            ['0']: {
