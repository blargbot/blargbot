    ...
    cases: [
        {
            ...
            setup(ctx) {
                ...
                ctx.discord.setup(m => m.editGuildMember(ctx.guild.id, ctx.users.command.id, $.looksLike({ roles: [ctx.roles.command.id, ctx.roles.other.id] }), 'Command User#0000'))
                    .thenResolve();
            }
        },
        {
        ...
        },
        {
            ...
            setup(ctx) {
                ...
                ctx.discord.setup(m => m.editGuildMember(ctx.guild.id, ctx.users.command.id, $.looksLike({ roles: [ctx.roles.command.id, ctx.roles.other.id, ctx.roles.bot.id] }), 'Command User#0000'))
                    .thenResolve();
            }
        },
        {
        ...
        },
        {
            ...
            setup(ctx) {
                ...
                ctx.discord.setup(m => m.editGuildMember(ctx.guild.id, ctx.users.other.id, $.looksLike({ roles: [ctx.roles.other.id, ctx.roles.bot.id] }), 'Command User#0000'))
                    .thenResolve();
            },
            postSetup(bbctx, ctx) {
