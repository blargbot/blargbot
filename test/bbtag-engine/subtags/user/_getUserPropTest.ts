...
export function* createGetUserPropTestCasesIter(options: GetUserPropTestData): Generator<SubtagTestCase, void, undefined> {
    ...
    yield {
        ...
        setup(ctx) {
            ...
            ctx.util.setup(m => m.findMembers($.isInstanceof(eris.Guild).and(g => g.id === ctx.guild.id).value, 'unknown user'))
                .verifiable(1)
                .thenResolve([]);
        }
    };
    ...
    return {
        ...
        postSetup(bbctx, ctx) {
            ...
            if (args[0] !== undefined && args[0] !== '') {
                ctx.util.setup(m => m.getUser(args[0]), false).thenResolve(undefined);
