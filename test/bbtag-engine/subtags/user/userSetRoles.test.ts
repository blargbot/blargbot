    ...
    cases: [
        {
            ...
            expected: 'true',
            setup(ctx) {
                ctx.discord.setup(m => m.editGuildMember(ctx.guild.id, ctx.users.command.id, $.looksLike({ roles: [] }), 'Command User#0000'))
            ...
            cases: [
                {
                    ...
                    postSetup(member, _, ctx) {
                        ctx.discord.setup(m => m.editGuildMember(ctx.guild.id, member.user.id, $.looksLike({ roles: [] }), 'Command User#0000'))
                            .thenResolve(member);
                    }
                }
            ]
            ...
            cases: [
                {
                    ...
                    postSetup(member, bbctx, ctx) {
                        ...
                        ctx.discord.setup(m => m.editGuildMember(ctx.guild.id, member.user.id, $.looksLike({ roles: ['283674284762348926', '234967249876489624'] }), 'Command User#0000'))
                            .thenResolve(member);
                    }
                }
            ]
