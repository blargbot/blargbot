...
runSubtagTests({
    ...
    cases: [
        ...
        {
            code: '{hash;invalid;{error}}',
            expected: '`Unsupported hash`',
            errors: [
                { start: 14, end: 21, error: new TestError(14) },
                { start: 0, end: 22, error: new BBTagRuntimeError('Unsupported hash', 'invalid is not a supported hash algorithm') }
            ]
        },
        {
            code: '{hash;{error};{error};{error}}',
