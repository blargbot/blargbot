...
async function migrateChangelog(discord: Discord, database: Database, logger: Logger): Promise<void> {
    const changelogs = await database.vars.get('changelog');
        ...
        try {
            ...
            if (channel !== null && guard.isTextableChannel(channel)) {
                try {
                    ...
                    await channel.send('Hi! Ive recently received an update to how changelogs work. This channel was subscribed, so I have migrated it to the new method!');
                } catch (ex: unknown) {
    ...
    await database.vars.set({ varname: 'changelog', guilds: {} });
