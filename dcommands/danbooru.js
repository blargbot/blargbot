...
e.execute = (msg, words) => {
    bu.isNsfwChannel(msg.channel.id).then(nsfwChannel => {
        ...
        delete tagList[0];
        bu.send('230801689551175681', `**__danbooru__:** tags: \`${tagList.join(' ')}\` **user:** ${msg.author.username} (${msg.author.id}) **channel:** ${msg.channel.name} (${msg.channel.id}) **guild:** ${msg.channel.guild.name} (${msg.channel.guild.id}) **NSFW Channel:** ${nsfwChannel}`);
