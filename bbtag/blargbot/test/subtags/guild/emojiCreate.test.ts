    ...
    cases: [
        {
            ...
            postSetup(bbctx, ctx) {
                ctx.dependencies.guild.setup(m => m.createEmote(bbctx.runtime, argument.isDeepEqual({
        ...
        },
        {
            ...
            postSetup(bbctx, ctx) {
                ...
                ctx.dependencies.roles.setup(m => m.querySingle(bbctx.runtime, '9128735617428916', argument.isDeepEqual({ noLookup: true }))).thenResolve(role);
                ctx.dependencies.roles.setup(m => m.querySingle(bbctx.runtime, 'abc', argument.isDeepEqual({ noLookup: true }))).thenResolve(undefined);
                ctx.dependencies.roles.setup(m => m.querySingle(bbctx.runtime, '', argument.isDeepEqual({ noLookup: true }))).thenResolve(undefined);
            }
        },
        {
            code: '{emojicreate;My cool emoji;data:image/png{semi}base64,abcdef}',
