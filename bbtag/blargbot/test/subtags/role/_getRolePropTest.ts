...
function* createGetRolePropTestCasesIter(options: GetRolePropTestData): Generator<SubtagTestCase, void, undefined> {
    ...
    yield {
        ...
        postSetup(bbctx, ctx) {
            ctx.inject.roles.setup(m => m.querySingle(bbctx.runtime, 'unknown role', argument.isDeepEqual(options.getQueryOptions?.(false)))).thenResolve(undefined);
        }
    };
    ...
    if (options.quiet !== false) {
        ...
        yield {
            ...
            postSetup(bbctx, ctx) {
                ctx.inject.roles.setup(m => m.querySingle(bbctx.runtime, 'unknown role', argument.isDeepEqual(options.getQueryOptions?.(true)))).thenResolve(undefined);
            }
        };
    }
}
    ...
    return {
        ...
        postSetup(bbctx, ctx) {
            ...
            if (args[0] !== undefined && args[0] !== '') {
                ctx.inject.roles.setup(m => m.querySingle(bbctx.runtime, args[0] as string, argument.isDeepEqual(data.getQueryOptions?.(isQuiet))), false).thenResolve(role);
            }
