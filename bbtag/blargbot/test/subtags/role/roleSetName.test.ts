    ...
    cases: [
            ...
            cases: [
                {
                    ...
                    postSetup(role, bbctx, ctx) {
                        ctx.inject.roles.setup(m => m.edit(bbctx.runtime, role.id, argument.isDeepEqual({ name: 'New name!' }))).thenResolve(undefined);
                    }
                }
            ]
        }),
        ...
        },
        {
            ...
            postSetup(bbctx, ctx) {
                ...
                ctx.inject.roles.setup(m => m.edit(bbctx.runtime, ctx.roles.bot.id, argument.isDeepEqual({ name: 'New name!' }))).thenResolve({ error: 'Test REST error' });
            }
        },
        {
            code: '{rolesetname;3298746326924;New name!}',
            ...
            postSetup(bbctx, ctx) {
                ctx.inject.roles.setup(m => m.querySingle(bbctx.runtime, '3298746326924', argument.isDeepEqual({ noLookup: false }))).thenResolve(ctx.roles.bot);
                ctx.inject.roles.setup(m => m.edit(bbctx.runtime, ctx.roles.bot.id, argument.isDeepEqual({ name: 'New name!' }))).thenResolve({ error: 'Some other error message' });
            }
        }
    ]
});
