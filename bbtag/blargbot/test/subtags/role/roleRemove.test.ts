    ...
    cases: [
        {
            ...
            postSetup(bbctx, ctx) {
                ...
                ctx.inject.users.setup(m => m.edit(bbctx.runtime, ctx.users.command.id, argument.isDeepEqual({ roles: [ctx.roles.everyone.id, ctx.roles.command.id, ctx.roles.bot.id] }))).thenResolve();
            }
        },
        {
            code: '{roleremove;3298746326924}',
        ...
        },
        {
            ...
            postSetup(bbctx, ctx) {
                ...
                ctx.users.command.member.roles.push(ctx.roles.other.id, ctx.roles.bot.id);
                ctx.inject.users.setup(m => m.edit(bbctx.runtime, ctx.users.command.id, argument.isDeepEqual({ roles: [ctx.roles.everyone.id, ctx.roles.command.id] }))).thenResolve();
            }
        },
        {
        ...
        },
        {
            ...
            postSetup(bbctx, ctx) {
                ...
                ctx.inject.users.setup(m => m.edit(bbctx.runtime, ctx.users.other.id, argument.isDeepEqual({ roles: [ctx.roles.everyone.id, ctx.roles.other.id, ctx.roles.authorizer.id] }))).thenResolve();
            }
        },
        {
            code: '{roleremove;3298746326924;other user}',
