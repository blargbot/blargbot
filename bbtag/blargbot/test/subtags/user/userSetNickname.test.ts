    ...
    cases: [
        {
            ...
            expected: '',
            postSetup(bbctx, ctx) {
                ctx.inject.users.setup(m => m.edit(bbctx.runtime, ctx.users.command.id, argument.isDeepEqual({ nick: 'abc' }))).thenResolve(undefined);
            }
        },
        {
            ...
            postSetup(bbctx, ctx) {
                ...
                ctx.inject.users.setup(m => m.edit(bbctx.runtime, userId, argument.isDeepEqual({ nick: 'abc' })))
                    .verifiable(1)
                    .thenResolve();
            }
        },
        {
            ...
            postSetup(bbctx, ctx) {
                ...
                const userId = randomUUID();
                member.setup(m => m.id).thenReturn(userId);
                ctx.inject.users.setup(m => m.querySingle(bbctx.runtime, 'blargbot'))
                    .verifiable(1)
                    .thenResolve(member.instance);
                ctx.inject.users.setup(m => m.edit(bbctx.runtime, userId, argument.isDeepEqual({ nick: 'abc' })))
                    .verifiable(1)
        ...
        },
        {
            ...
            postSetup(bbctx, ctx) {
                ctx.inject.users.setup(m => m.querySingle(bbctx.runtime, 'unknown user'))
                    .verifiable(1)
