    ...
    cases: [
            ...
            cases: [
                {
                    ...
                    postSetup(role, bbctx, ctx) {
                        ctx.dependencies.role.setup(m => m.edit(bbctx, role.id, argument.isDeepEqual({ mentionable: true }))).thenResolve(undefined);
                    }
                }
            ]
        }),
            ...
            cases: [
                {
                    expected: '',
                    postSetup(role, bbctx, ctx) {
                        ctx.dependencies.role.setup(m => m.edit(bbctx, role.id, argument.isDeepEqual({ mentionable: false }))).thenResolve(undefined);
                    }
                }
            ]
        }),
        ...
        },
        {
            ...
            postSetup(bbctx, ctx) {
                ...
                ctx.dependencies.role.setup(m => m.edit(bbctx, ctx.roles.bot.id, argument.isDeepEqual({ mentionable: true }))).thenResolve({ error: 'Test REST error' });
            }
        },
        {
            code: '{rolesetmentionable;3298746326924}',
            ...
            postSetup(bbctx, ctx) {
                ctx.dependencies.role.setup(m => m.querySingle(bbctx, '3298746326924', argument.isDeepEqual({ noLookup: false }))).thenResolve(ctx.roles.bot);
                ctx.dependencies.role.setup(m => m.edit(bbctx, ctx.roles.bot.id, argument.isDeepEqual({ mentionable: true }))).thenResolve({ error: 'Some other error message' });
            }
        }
    ]
});
