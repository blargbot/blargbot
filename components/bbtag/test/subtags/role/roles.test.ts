...
import { RolesSubtag } from '@bbtag/blargbot/subtags';

import { runSubtagTests } from '../SubtagTestSuite.js';
import { createGetUserPropTestCases } from '../user/_getUserPropTest.js';

runSubtagTests({
    subtag: Subtag.getDescriptor(RolesSubtag),
            ...
            cases: [
                ...
                },
                {
                    ...
                    setup(member, ctx) {
                        if (member.member === undefined)
                            throw new Error('User isnt member of guild');
                        ctx.roles.other.id = '098765434512212678';
                ...
                },
                {
                    ...
                    setup(member, ctx) {
                        if (member.member === undefined)
                            throw new Error('User isnt member of guild');
                        ctx.roles.top.id = '098765434512212678';
                        ctx.roles.other.id = '1234567890987654';
