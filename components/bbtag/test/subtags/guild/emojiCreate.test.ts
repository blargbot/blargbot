    ...
    cases: [
        {
            ...
            postSetup(bbctx, ctx) {
                ctx.guildService.setup(m => m.createEmote(bbctx, argument.isDeepEqual({
        ...
        },
        {
            ...
            postSetup(bbctx, ctx) {
                ...
                ctx.roleService.setup(m => m.querySingle(bbctx, '9128735617428916', argument.isDeepEqual({ noLookup: true }))).thenResolve(role);
                ctx.roleService.setup(m => m.querySingle(bbctx, 'abc', argument.isDeepEqual({ noLookup: true }))).thenResolve(undefined);
                ctx.roleService.setup(m => m.querySingle(bbctx, '', argument.isDeepEqual({ noLookup: true }))).thenResolve(undefined);
            }
        },
        {
            code: '{emojicreate;My cool emoji;data:image/png{semi}base64,abcdef}',
